...
const os$1 = __importStar$3(require$$0$1);
...
// We use any as a valid input type
/* eslint-disable @typescript-eslint/no-explicit-any */
const fs$1 = __importStar$2(fs$2);
const os = __importStar$2(require$$0$1);
const uuid_1 = dist;
...
const isCloned = (workDir) => fs$2.existsSync(path$1.resolve(workDir, '.git'));
...
const useNpm = (workDir, pkgManager = '') => 'npm' === pkgManager ||
    ('yarn' !== pkgManager && (fs$2.existsSync(path$1.resolve(workDir, 'package-lock.json')) ||
        !fs$2.existsSync(path$1.resolve(workDir, 'yarn.lock'))));
...
var httpAdapter = isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    const emitter = new require$$4$1.EventEmitter();
...
const getTargetRunId = (context) => /^\d+$/.test(coreExports.getInput('TARGET_RUN_ID')) ? Number(coreExports.getInput('TARGET_RUN_ID')) : context.runId;
...
const execute = async (logger, octokit, context) => {
    ...
    if (envName) {
        coreExports.exportVariable(envName, conclusion);
    }
};
...
const run = async () => {
    ...
    await execute(logger, utils$2.getOctokit(), context);
};
run().catch(error => {
    console.log(error);
